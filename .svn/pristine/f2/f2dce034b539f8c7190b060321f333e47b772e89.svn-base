...
com.viewModel.edit = function (data) {
    ...
    this.saveClick = function () {
        ...
        if ($("#agreeBox").length>0) {
            if ($("#agreeBox").is(':checked')) {
                ...
                com.message('warning', "请注册前确认您同意协议");
                return;
            }
        }
        //取得数据
    ...
    this.fnIsPageValid = function (context) {
        var formValid = com.formValidate(context);
        if (!formValid)
            return '验证不通过，数据未保存！';
    ...
    this.init();
    this.fnIsAudit = function () { return (data.dataSource.pageData.form != null && ("ApproveState" in data.dataSource.pageData.form) && self.pageData.form["ApproveState"]() == "passed"); };
