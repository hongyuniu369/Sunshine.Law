...
**/
var news = news || {};
news.viewModel = com.viewModel || {};



news.viewModel = function (data) {
    var self = this;
    ...
    this.tree = {
        method: 'GET',
        url: '/api/cms/newsclass/GetAll',
        queryParams: ko.observable(),
        loadFilter: function (d) {
            var filter = utils.filterProperties(d.rows || d, ['ClassCode as id', 'ClassName as text']);
            return [{ id: '', text: '所有栏目', children: filter }];
        },
        onLoadSuccess: function () {
            var treeElemet = $(self.tree.$element()[0]);
            treeElemet.tree("select", $("div[node-id='" + self.pageData.form.ClassCode() + "']"));
            self.ParentClassCode(data.dataSource.pageData.form.ClassCode);
        },
    ...
    this.ParentClassCode.subscribe(function (value) {//设置树形和新闻所属栏目联动
        self.pageData.form.ClassCode(value);
    });
    ...
    this.deleteClick = function () {
        if (self.fnIsNew()) { return com.message('warning', '数据尚未保存，无需删除！'); }
        com.message('confirm', self.resx.deleteConfirm, function (b) {
            if (b) {
                com.ajax({
                    type: 'DELETE', url: self.urls.remove + data.dataSource.pageData.form[self.form.primaryKeys[0]], success: function () {
    ...
    //this.fnIsAudit = ko.computed(function() {
    //    if (data.dataSource.pageData.form && data.dataSource.pageData.form.ApproveState)
