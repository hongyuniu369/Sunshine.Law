...
namespace Zephyr.Controllers
{
    ...
    public class H5Controller : Controller
    {
        ...
        public ActionResult list(RequestWrapper query)
        {
            ...
            model.page.urls = new { query = "/h5/list" };
...
<settings defaultOrderBy='AdvisoryCode desc'>
    ...
    public class H5ApiController : ApiController
    {
        [System.Web.Http.AllowAnonymous]
        [MvcMenuFilter(false)]
        public System.Net.Http.HttpResponseMessage GetJsonp(RequestWrapper query)
        {
            query.LoadSettingXmlString(@"
<settings defaultOrderBy='AdvisoryCode desc'>
    <select>A.AdvisoryCode, A.MsgTitle, A.MsgInDate, C.ClassName</select>
    <from>msg_advisory as A 
        left outer join msg_class C on A.MsgClass=C.ClassCode
    </from>
    <where defaultForAll='false' defaultCp='equal' defaultIgnoreEmpty='true' >
        ...
        <field name='MsgClass'      cp='mapalias'       extend='ClassCode,law_lawyerClassMap,LawyerCode'    variable='Lawyer'   ignoreEmpty='true'></field> 
            ...
            string sResult = callback + "(" + Newtonsoft.Json.JsonConvert.SerializeObject(result) + ")";
            System.Net.Http.HttpResponseMessage result1 = new System.Net.Http.HttpResponseMessage { Content = new System.Net.Http.StringContent(sResult, System.Text.Encoding.GetEncoding("UTF-8"), "text/plain") };
            return result1;
        }
    }
}
