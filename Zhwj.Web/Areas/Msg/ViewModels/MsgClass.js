...
var viewModel = function(){
    ...
    this.tree = {
        method:'GET',
        url: '/api/msg/msgclass/GetRootMsgClass',
    ...
    this.addClick = function () {
        ...
        if (self.grid.onClickRow()) {
            com.ajax({
                type: 'GET',
                url: '/api/msg/msgclass/getnewcode',
                success: function (d) {//设置新增行默认属性
                    var row = { ParentClassCode: self.ParentClassCode(), ClassCode: d, IsEnable: true };
    ...
    this.saveClick = function () {
        self.gridEdit.ended();
        var post = {};
        post.list = self.gridEdit.getChanges(['ClassCode', 'ClassName', 'ParentClassCode', 'ClassRemark', 'IsEnable', 'ClassOrder']);
        if (self.gridEdit.isChangedAndValid()) {
            com.ajax({
                url: '/api/msg/msgclass/edit',
                data: ko.toJSON(post),
                success: function (d) {
            ...
            viewModel: function (w) {
                ...
                this.addTypeClick = function () {
                    com.ajax({
                        ...
                        url: '/api/msg/msgclass/getnewcode',
                        success: function (d) {//设置新增行默认属性
                            var row = { ClassCode: d, IsEnable:'true', ClassOrder: 99};
                ...
                this.confirmClick = function () {
                    if (that.typeGridEdit.isChangedAndValid()) {
                        var list = that.typeGridEdit.getChanges(['ClassCode', 'ClassName',  'ClassRemark', 'IsEnable', 'ClassOrder']);
                        com.ajax({
                            url: '/api/msg/msgclass/edit/',
