...
order by ButtonSeq";
        ...
        public object MultyResult()
        {
            using (var commond = db.MultiResultSql)
            {
                List<sys_button> buttons = commond.Sql(
                    @"select top(10) * from sys_button; 
                    select top(10) * from sys_menu").QueryMany<sys_button>();

                IList<sys_menu> menus = commond.QueryMany<sys_menu>();
