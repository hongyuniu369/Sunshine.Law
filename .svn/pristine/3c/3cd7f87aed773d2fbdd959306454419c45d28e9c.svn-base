...
namespace Zephyr.Core
{
    ...
    public class NewKey
	{
        ...
        //带前缀N位最大值加一
        public static string prefix(IDbContext db, string table, string field, string prefix, int numberLength, ParamQuery pQuery)
        {
            //var where = pQuery.GetData().WhereSql;
            var sqlWhere = " where " + field + " like '" + prefix + "%'";
            if (pQuery != null)
                sqlWhere += " and " + pQuery.GetData().WhereSql;
            var dbkey = db.Sql(String.Format("select isnull(max({0}),0) from {1} {2}", field, table, sqlWhere)).QuerySingle<string>();
            var mykey = string.Empty.PadLeft(numberLength, '0');
            var cachedKeys = getCacheKey(table, field);
            dbkey = dbkey.Length <= prefix.Length ? "" : dbkey.Remove(0, prefix.Length);
            cachedKeys = cachedKeys.Length <= prefix.Length ? "" : cachedKeys.Remove(0, prefix.Length);
            var currentKey = maxOfAllKey(cachedKeys, ZConvert.ToString(dbkey), mykey);
            var key = prefix + ZConvert.ToString(currentKey + 1).PadLeft(numberLength, '0');
            SetCacheKey(table, field, key);
            return key;
        }
