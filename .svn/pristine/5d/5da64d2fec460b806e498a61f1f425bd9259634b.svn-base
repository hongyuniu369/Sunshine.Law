...
namespace Zephyr.Areas.CMS
{
    public class CMSHelper
    {
        ...
        public static dynamic GetEditUrls(string controller, object extend = null)
        {
            ...
            expando["getdetail"] = string.Format("/api/CMS/{0}/getdetail/", controller);
            expando["getmaster"] = string.Format("/api/CMS/{0}/geteditmaster/", controller);
            expando["edit"] = string.Format("/api/CMS/{0}/edit/", controller);
            expando["audit"] = string.Format("/api/CMS/{0}/audit/", controller);
            expando["getrowid"] = string.Format("/api/CMS/{0}/getnewrowid/", controller);
            expando["report"] = controller;
            expando["remove"] = string.Format("/api/CMS/{0}/delete/", controller);
        ...
        {
            var expando = (IDictionary<string, object>)new ExpandoObject();
            expando["query"] = string.Format("/api/CMS/{0}", controller);
            expando["remove"] = string.Format("/api/CMS/{0}/", controller);
            expando["newcode"] = string.Format("/api/CMS/{0}/getnewcode", controller);
            expando["audit"] = string.Format("/api/CMS/{0}/audit/", controller);
            expando["edit"] = string.Format("/CMS/{0}/edit/", controller);
            if (extend != null)
                EachHelper.EachObjectProperty(extend, (i, name, value) => { expando[name] = value; });
