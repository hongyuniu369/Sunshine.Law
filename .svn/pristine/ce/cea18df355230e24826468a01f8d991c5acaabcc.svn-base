...
namespace Zephyr.Models
{
    public class sys_userService : ServiceBase<sys_user>
    {
        public object Login(JObject request) 
        {
            ...
            var roles = GetUserRoles(result.UserCode).Select(r => r.RoleCode).ToArray();

            //调用框架中的登陆机制
            var loginer = new LoginerBase { UserCode = result.UserCode, UserName = result.UserName, Roles = roles}; 
        ...
        public object UpdatePassword(JObject request)
        {
            var UserCode = request.Value<string>("UserCode");
            var OldPassword = request.Value<string>("OldPassword");
            var NewPassword = request.Value<string>("NewPassword");
            
            //用户名密码检查
            if(UserCode != FormsAuth.GetUserData().UserCode)
                return new { status = "error", message = "用户名或密码不正确！" };
            if (String.IsNullOrEmpty(UserCode) || String.IsNullOrEmpty(OldPassword))
                return new { status = "error", message = "用户名或原密码不能为空！" };
            ...
            //更新密码
            var ru = this.Update(ParamUpdate.Instance().Column("Password", NewPassword).AndWhere("UserCode", UserCode));
            if (ru <= 0)
                return new { status = "error", message = "修改失败，请联系管理员！" };
...
left join sys_userOrganizeMap B on B.OrganizeCode = A.OrganizeCode and B.UserCode = '{0}'", user);
        ...
        public List<sys_role> GetUserRoles(string user)
        {
            var sql = String.Format(@"
select distinct A.*
from sys_role A
inner join sys_userRoleMap B on B.RoleCode = A.RoleCode and B.UserCode = '{0}'", user);
            return db.Sql(sql).QueryMany<sys_role>();
        }
