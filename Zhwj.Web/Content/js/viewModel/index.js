...
wrapper.changePassword = function () {
    com.dialog({
        ...
        viewModel: function (w) {
            w.find("[name=UserCode]").val(com.cookie('CurrentProjectName'));
            w.find("#pwd_confirm").click(function () {
                ...
                var oldPassword = w.find("[name=OldPassword]").val();
                var newPassword = w.find("[name=NewPassword]").val();
                com.ajax({
                    type: 'POST',
                    url: "/api/sys/user/UpdatePassword/",
                    data: JSON.stringify({ UserCode: userCode, OldPassword: oldPassword, NewPassword: newPassword }),
                    success: function (d) {
                        if (d.status == 'success') {
                            w.dialog('close');
...
wrapper.tabRefresh = function (url) {
    ...
    var currentTab = $tab.tabs('getSelected');
    var src = $(currentTab.panel('options').content).attr('src');
    if (typeof src != 'string' && typeof url == 'string') src = url;
    if(src)
        $tab.tabs('update', { tab: currentTab, options: { content: wrapper.createFrame(src) } })
};

wrapper.tabCloseCurrent = function () {
    var $tab = $("#tabs");
    var currentTab = $tab.tabs('getSelected');
    var currtab_title = currentTab.panel('options').title;
    $tab.tabs('close', currtab_title);
...
wrapper.setLocationHash = function () {
    try {
        ...
        var homeUrl = wrapper.settings.homeTabUrl;
