...
com.formatCheckbox = function (value) {
    var checked = (value || 'false').toString() == 'true' || (value || '').toString() == 'passed';
    return utils.formatString('<img value={0} src="/Content/images/{1}"/>', checked, checked ? "checkmark.gif" : "checknomark.gif");
};
//格式化审核状态
com.formatAudit = function (value) {
    var picname = "checknomark.gif";
    if ((value || 'false').toString() == 'true' || (value || '').toString() == 'passed')
        picname = 'checkmark.gif'
    if ((value || '').toString() == 'false' || (value || '').toString() == 'reject')
        picname = 'checkcross.gif'
    return utils.formatString('<img value={0} src="/Content/images/{1}"/>', value, picname);
};
...
//关闭当前tab页签
com.closeCurrentTab = function () {
    parent.wrapper.tabCloseCurrent.apply(this, arguments);
}
...
com.auditDialogForEditVM = function () {
    var query = parent.$;
    var winAudit = query('#w_audit_div_foreditvm'), args = arguments;
    if (winAudit.length == 0) {
        var html = utils.functionComment(function () {/*
            <div id="w_audit_wrapper">
                <div id="w_audit_div_foreditvm" class="easyui-dialog"  title="审核" data-options="modal:true,closed:true,iconCls:'icon-user-accept'" style="width:400px;height:210px;" buttons="#w_audit_div_foreditvm_button"> 
                ...
                <div id="w_audit_div_foreditvm_button" class="audit_button">  
