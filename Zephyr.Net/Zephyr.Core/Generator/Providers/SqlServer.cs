...
namespace Zephyr.Core.Generator
{
    ...
    public class SqlServerGen : ISqlGen
    {
        ...
        {
            return "SELECT a.Name as TableName, isnull(g.[value],a.Name) AS TableDescription FROM sys.tables a left join sys.extended_properties g on (a.object_id = g.major_id AND g.minor_id = 0) order by a.Name";
        }
