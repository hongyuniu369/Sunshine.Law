...
namespace Zephyr.Areas.Sys.Controllers
{
    ...
    public class GeneratorApiController : ApiController
    {
        ...
        public void Post(JToken data)
        {
            ...
            var tableDescription = data.Value<string>("tableDescription");
            ...
            using (var db = Db.Context(database))
            {
                ...
                var codePath = path + area + "/";
                if (type.ToLower() == "edit")
                {
                    ZFiles.SaveStreamToFile(GetStreamFromString(modelCode), GetFilePath(codePath + "/Models/", table + "Edit.cs"));
                    ZFiles.SaveStreamToFile(GetStreamFromString(controllerCode), GetFilePath(codePath + "Controllers/", controller + "ControllerEdit.cs"));
                    ZFiles.SaveStreamToFile(GetStreamFromString(viewCode), GetFilePath(codePath + "Views/" + controller + "/", "Edit.cshtml"));
                }
                else
                {
                    ZFiles.SaveStreamToFile(GetStreamFromString(modelCode), GetFilePath(codePath + "Models/", table + ".cs"));
                    ZFiles.SaveStreamToFile(GetStreamFromString(controllerCode), GetFilePath(codePath + "Controllers/", controller + "Controller.cs"));
                    ZFiles.SaveStreamToFile(GetStreamFromString(viewCode), GetFilePath(codePath + "Views/" + controller + "/", "Index.cshtml"));
                }
