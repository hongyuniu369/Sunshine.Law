...
namespace Zephyr.Core
{
    ...
    public partial class ServiceBase<T> where T : ModelBase, new()
    {
        ...
        public string GetNewKey(string field,string rule,int qty = 1,ParamQuery pQuery=null, string prefix="CODE")
        {
                ...
                for (var i = 0; i < qty; i++)
                {
                    ...
                    switch (rule)
                    {
                        case "dateplus":
                            ...
                            break;
                        case "prefix":
                            newkey = NewKey.prefix(db, table, field, prefix, 4, pQuery);
                            break;
                        case "guid":
                            newkey = NewKey.guid();
                            break;
