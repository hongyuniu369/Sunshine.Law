...
<settings defaultOrderBy='UserSeq'>
   <where>
        <field name='UserCode' cp='like' variable='value' ignoreEmpty='true'></field>
        <field name='UserName' cp='like' variable='text' ignoreEmpty='true'></field>
        ...
        <field name='UserCode' cp='map' extend='sys_userRoleMap,RoleCode' variable='RoleCode' ignoreEmpty='true'></field>
    </where>
</settings>");
        ...
        public dynamic UpdatePassword(JObject request)
        {
            var message = new sys_userService().UpdatePassword(request);
            return message;
            //return new Newtonsoft.Json.JsonConvert.SerializeObject(message);
        }
